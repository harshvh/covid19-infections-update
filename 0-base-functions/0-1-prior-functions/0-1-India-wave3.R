...
# p2 := P(S|untested)
samp_p2 <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0.001,b = 0.2,
                    shape1 = find_beta_shape_params(mu = 0.1, sd = (0.2)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.1, sd = (0.2)^2)$b)
}

# Z_s := adjustment factor for P(test +|S) among untested (alpha)
samp_zs <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0.4, b = 0.85,
...
samp_za <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0.02,b = 0.35,
                    shape1 = find_beta_shape_params(mu = 0.15, sd = (0.2)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.15, sd = (0.2)^2)$b)
}
...
##############################################
p0 <- function(x){
  truncdist::dtrunc(x = x,spec = "beta",a = 0.8,b = 1,
                    shape1 = find_beta_shape_params(mu = 0.95, sd = (0.2)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.95, sd = (0.2)^2)$b)
}
