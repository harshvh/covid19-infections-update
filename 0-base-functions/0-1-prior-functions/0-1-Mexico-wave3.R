...
# p1 := P(S|tested)
samp_p1 <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0.75,b = 1,
                    shape1 = find_beta_shape_params(mu = 0.875, sd = (0.1)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.875, sd = (0.1)^2)$b)
}

# p2 := P(S|untested)
samp_p2 <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0,b = 0.21,
                    shape1 = find_beta_shape_params(mu = 0.08, sd = (0.1)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.08, sd = (0.1)^2)$b)
}

# Z_s := adjustment factor for P(test +|S) among untested (alpha)
samp_zs <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0.4,b = 0.8,
...
dist_Se = truncdist::rtrunc(n = 100000,spec = "beta",a = 0.8,b = 1,
                            shape1 = find_beta_shape_params(mu = 0.9, sd = (0.3)^2)$a,
                            shape2 = find_beta_shape_params(mu = 0.9, sd = (0.3)^2)$b)
...
p0 <- function(x){
  truncdist::dtrunc(x = x,spec = "beta",a = 0.4,b = 0.8,
                    shape1 = find_beta_shape_params(mu = 0.5, sd = (0.2)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.5, sd = (0.2)^2)$b)
}
