...
# p1 := P(S|tested)
samp_p1 <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0.5,b = 1,
...
# p2 := P(S|untested)
samp_p2 <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0,b = 0.17,
                    shape1 = find_beta_shape_params(mu = 0.055, sd = (0.2)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.055, sd = (0.2)^2)$b)
}

# Z_s := adjustment factor for P(test +|S) among untested (alpha)
samp_zs <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0.65,b = 1,
                    shape1 = find_beta_shape_params(mu = 0.7, sd = (0.1)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.7, sd = (0.1)^2)$b)
}

# Z_a := adjustment factor for P(test +|A) among untested (beta)
samp_za <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0,b = 0.45,
...
##############################################
p0 <- function(x){
  truncdist::dtrunc(x = x,spec = "beta",a = 0.25,b = 0.6,
                    shape1 = find_beta_shape_params(mu = 0.3, sd = (0.2)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.3, sd = (0.2)^2)$b)
}
