...
samp_p1 <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0.75,b = 1,
                    shape1 = find_beta_shape_params(mu = 0.9, sd = (0.01)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.9, sd = (0.01)^2)$b)
}

# p2 := P(S|untested)
samp_p2 <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0.001,b = 0.1,
                    shape1 = find_beta_shape_params(mu = 0.02, sd = (0.1)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.02, sd = (0.1)^2)$b)
}

# Z_s := adjustment factor for P(test +|S) among untested (alpha)
samp_zs <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0.2,b = 0.6,
                    shape1 = find_beta_shape_params(mu = 0.25, sd = (0.1)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.25, sd = (0.1)^2)$b)
}

# Z_a := adjustment factor for P(test +|A) among untested (beta)
samp_za <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0.01,b = 0.3,
                    shape1 = find_beta_shape_params(mu = 0.065, sd = (0.1)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.065, sd = (0.1)^2)$b)
}
...
##############################################
p0 <- function(x){
  truncdist::dtrunc(x = x,spec = "beta",a = 0.45,b = 0.95,
                    shape1 = find_beta_shape_params(mu = 0.8, sd = (0.05)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.8, sd = (0.05)^2)$b)
}
