...
samp_p1 <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0.8,b = 1,
                    shape1 = find_beta_shape_params(mu = 0.9, sd = (0.125)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.9, sd = (0.125)^2)$b)
}

# p2 := P(S|untested)
samp_p2 <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0.01,b = 0.4,
                    shape1 = find_beta_shape_params(mu = 0.05, sd = (0.2)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.05, sd = (0.2)^2)$b)
}

# Z_s := adjustment factor for P(test +|S) among untested (alpha)
samp_zs <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0.35,b = 0.55,
                    shape1 = find_beta_shape_params(mu = 0.425, sd = (0.15)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.425, sd = (0.15)^2)$b)
}

# Z_a := adjustment factor for P(test +|A) among untested (beta)
samp_za <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0.04,b = 0.3,
...
##############################################
p0 <- function(x){
  truncdist::dtrunc(x = x,spec = "beta",a = 0.45,b = 0.7,
                    shape1 = find_beta_shape_params(mu = 0.65, sd = (0.2)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.65, sd = (0.2)^2)$b)
}
