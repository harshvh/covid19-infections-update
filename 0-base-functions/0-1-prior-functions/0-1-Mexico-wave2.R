...
# Prior Functions #
# Mexico
# Wave 2 - 01.06.2020 to 31.10.2020
...
# p1 := P(S|tested)
samp_p1 <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0.85,b = 1,
                    shape1 = find_beta_shape_params(mu = 0.925, sd = (0.05)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.925, sd = (0.05)^2)$b)
}

# p2 := P(S|untested)
samp_p2 <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0,b = 0.15,
                    shape1 = find_beta_shape_params(mu = 0.06, sd = (0.05)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.06, sd = (0.05)^2)$b)
}

# Z_s := adjustment factor for P(test +|S) among untested (alpha)
samp_zs <- function(n){
  truncdist::rtrunc(n = n,spec = "beta",a = 0.25,b = 0.5,
                    shape1 = find_beta_shape_params(mu = 0.3, sd = (0.1)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.3, sd = (0.1)^2)$b)
}
...
##############################################
p0 <- function(x){
  truncdist::dtrunc(x = x,spec = "beta",a = 0.4,b = 0.9,
                    shape1 = find_beta_shape_params(mu = 0.65, sd = (0.1)^2)$a,
                    shape2 = find_beta_shape_params(mu = 0.65, sd = (0.1)^2)$b)
}
