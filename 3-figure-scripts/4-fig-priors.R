...
# Priors Figure #
# --> Would need to input prior values for each prior and each coutnry separately into the code.
################################################################################
rm(list=ls())
source(paste0(here::here(), "/0-config.R"))


priors = readRDS(paste0(results_path, "India_2021-02-28_priors_out.RDS"))

state_abbrev <- read_csv(country_abbrev_path) %>%
